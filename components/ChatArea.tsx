...
function DMArea({ conversationId }: { conversationId: string }) {
  ...
  useEffect(() => {
    ...
    // Check if we're accepting a call from AppShell
    const acceptingCallId = sessionStorage.getItem('acceptingCall');
    if (acceptingCallId === conversationId) {
      console.log("📞 Auto-accepting call for conversation:", conversationId);
      sessionStorage.removeItem('acceptingCall');
      setInCall(true);
    }
  ...
  const handleStartCall = async () => {
    ...
    try {
      ...
      // Get the existing channel reference (don't create a new subscription)
      ...
      // Open call UI immediately for caller
      setInCall(true);
    } catch (error) {
